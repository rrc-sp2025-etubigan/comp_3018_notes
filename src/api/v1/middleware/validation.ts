...
export const validateRequest = (
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            // Validates body if schema.body is provided
            if (schemas.body) {
            ...
            // Validates params if schema.param is provided
            if (schemas.params) {
                const value = validatePart(
                    schemas.params,
                    req.params,
            ...
            // Validates query if schema.query is provided
            if (schemas.query) {
                const value = validatePart(
                    schemas.query,
                    req.query,
            ...
            // If any errors were caught, respond with appropriate HTTP status code
            if (errors.length > 0) {
